...
for idx, texture_name in enumerate(classes):  # 각 class 마다
    ...
    for image_name in os.listdir(image_dir):  # 경로에 있는 모든 이미지에 대해
        ...
        for _ in range(10):  # 이미지에서 random하게 10개 패치 자르기
            ...
            X_train.append(image_p)  # laws texture 특징 추가
...
for idx, texture_name in enumerate(classes):  # 각 class 마다
    ...
    for image_name in os.listdir(image_dir):  # 경로에 있는 모든 이미지에 대해
        ...
        X_test.append(image)  # laws texture 특징 추가
...
class MLP(nn.Module):  # MLP class
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim):  # 초기화 함수
        super(MLP, self).__init__()  # 기반 클래스 nn.Module을 초기화
        self.fc1 = nn.Linear(input_dim, hidden_dim1)  # input_dim x hidden_dim1
        self.fc2 = nn.Linear(hidden_dim1, hidden_dim1)  # hidden_dim1 x hidden_dim1
        self.fc3 = nn.Linear(hidden_dim1, hidden_dim2)  # hidden_dim1 x hidden_dim2
        self.fc4 = nn.Linear(hidden_dim2, hidden_dim2)  # hidden_dim2 x hidden_dim2
        self.fc5 = nn.Linear(hidden_dim2, output_dim)  # hidden_dim2 x output_dim
    ...
    def forward(self, x):  # x: input_dim
        ...
        out = self.relu(out)  # out: hidden_dim
        out = self.fc4(out)  # out: output_dim
        out = self.relu(out)  # out: hidden_dim
...
net = MLP(32*32*3, 1024, 128, 3)  # MLP 모델 정의
