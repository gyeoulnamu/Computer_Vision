...
for idx, texture_name in enumerate(classes):  # 각 class 마다
    ...
    for image_name in os.listdir(image_dir):  # 경로에 있는 모든 이미지에 대해
        ...
        image_s_gray = cv2.cvtColor(image_s, cv2.COLOR_BGR2GRAY)  # 이미지를 흑백으로 변환
        glcm = greycomatrix(image_s_gray, distances=[1], angles=[np.pi/2], levels=256, symmetric=False, normed=True)  # GLCM co-occurrence 계산
        X_train.append([greycoprops(glcm, 'contrast')[0, 0], greycoprops(glcm, 'correlation')[0, 0]] + laws_texture(image_s_gray))  # GLCM dissimilarity, correlation 특징 추가 (2차원) + laws texture 특징 추가 (9차원)
...
for idx, texture_name in enumerate(classes):
    ...
    for image_name in os.listdir(image_dir):
        ...
        image_s_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        glcm = greycomatrix(image_s_gray, distances = [1], angles=[np.pi/2], levels=256, symmetric=False, normed=True)
        X_test.append([greycoprops(glcm, 'contrast')[0, 0], greycoprops(glcm, 'correlation')[0, 0]] + laws_texture(image_s_gray))
        Y_test.append(idx)
