...
for idx, texture_name in enumerate(classes):  # 각 class 마다
    ...
    for image_name in os.listdir(image_dir):  # 경로에 있는 모든 이미지에 대해
        ...
        for _ in range(10):  # 이미지에서 random하게 10개 패치 자르기
            ...
            X_train.append([greycoprops(glcm, 'dissimilarity')[0, 0], greycoprops(glcm, 'correlation')[0, 0]] + laws_texture(image_p_gray))  # GLCM dissimilarity, correlation 특징 추가 (2차원) + laws texture 특징 추가 (9차원)
